document.getElementById("bitcoinForm").addEventListener("submit",function(n){n.preventDefault();var i=n.target,r=new FormData(i),t={};r.forEach((n,i)=>{t[i]=n});fetch("/Bitcoin/GetBitcoinPrice",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}).then(n=>n.json()).then(n=>{var t=document.getElementById("result");t.innerHTML=n.success?`
            ...
            </div>`:`<p class="text-danger">Erro: ${n.error}</p>`}).catch(n=>{console.error("Erro:",n)})});